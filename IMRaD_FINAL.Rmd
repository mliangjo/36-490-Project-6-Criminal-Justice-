...
The Dirichlet distribution is a multivariate generalization of the Beta distribution used to model vectors of proportions. Let $Y_i = (y_{i1}, \dots, y_{ik})$ denote a vector of category shares for observation $i$, where $k$ is the number of categories. We require that each component is nonnegative and the components sum to one. In notation, $y_{ij} \ge 0$ and $\sum_{j=1}^ky_{ij} = 1$. This makes the Dirichlet distribution well-suited for compositional data.
...
As the total concentration parameter $\sum_j \alpha_{ij}$ increases, the variance decreases and the distribution becomes more concentrated around its mean. Larger values of $\alpha_{ij}$ relative to other components increase the expected share of category $j$. 
...
After conducting EDA, we found that the conviction rate varies systematically across PA, OR, and FL. To provide a comparison, we examine public defender attorney cases in Pennsylvania through utilizing logistic regression.
...
For public defenders, the Number of Criminal Court Judges shows to be statistically significant ($\beta$ = 0.009, p<0.1). The model’s overall fit remains low with $R^2_a$ = 0.021. Note we use the adjusted $R^2_a \coloneq 1 - \dfrac{SS_{\text{res}}/df_{\text{res}}} {SS_{\text{tot}}/df_{\text{tot}}}$, for $SS$ the sum of square errors and $df$ the degrees of freedom as a measure of determination which adjusts for the degrees of freedom used. For the private attorney type, Poverty Rate and Number of Criminal Court Judges show to be statistically significant (Poverty Rate: $\beta$ = 0.074, p<0.05, Number of Criminal Court Judges: $\beta$ = 0.013, p<0.01). Compared to the other models, the private attorney model shows the highest explanatory power, although the overall fit is still modest ($R^2_a$ = 0.131). For the court-appointed attorney type, none of the explanatory variables are statistically significant. The model also shows to be a poor fit with a negative $R^2_a$ (-0.075). For the self-represented attorney type, the Proportion Hispanic or Latino shows to be statistically significant ($\beta$ = -0.368, p<0.05). However, this model has the smallest sample size and a negative adjusted $R^2_a$ (-0.029), indicating low explanatory power.
...
```
\begin{center}
\textbf{Figure 3.2.3:} Cook’s distance plot for the conviction rate model (PA - Public Defender)
\end{center}
...
```
\begin{center}
\textbf{Figure 3.2.4:} Predicted Conviction Rate vs. Unemployment Rate by Attorney Type  (Pennsylvania)
\end{center}
...
Across attorney types, the alignment between observed points and predicted curves is generally weak, consistent with the low $R^2_a$ values reported in Figure 3.2.1. For court-appointed and private counsel the predicted trends are relatively flat and do not closely track the distribution of observed values, suggesting limited explanatory power. The public defender model shows a slightly more visible pattern, with a modest negative relationship between unemployment rate and predicted conviction rate, though much variability remains in the model’s predictions.

For self-represented defendants, the predicted curve exhibits a more pronounced nonlinear pattern, however, the wide spread of observed points, particularly at higher unemployment rates, indicate instability in the model fit. This is further reflected in the large uncertainty and low adjusted $R^2_a$ for this model.

On each of these plots, we can quite clearly see three counties with extreme unemployment rates, which likely have high influence on the fitting of this model. This corresponds with our Cook's distance analysis as shown in Figure 3.2.3, which identified three counties with a larger influence on the model fit.

\newpage

## Counsel Type Composition Regression

We also found from our exploratory data analysis that the proportional share of legal representation varies systematically across PA, OR, and FL. We will examine in detail the variation in counsel type distribution across counties in Pennsylvania. As outlined above, we will utilize Dirichlet regression to create a model for the response variable which is a vector of counsel type proportions per county and use county characteristics as the covariates.
...
```
\begin{center}
\textbf{Figure 3.3.1:} Counsel Type Composition Regression Results (Pennsylvania, Common Parameterization)
\end{center}

Note: See also Figure A.3.4 in the appendix for an alternative parameterization of this model. 

Figure 3.3.1 displays some incredibly notable patterns in the model.
...
Looking across all three states, the regression results reveal that no single county level covariate consistently predicts counsel type distributions, though several noteworthy patterns emerge.
...
### Table of Counsel Type Percentage Regression Results (Common Parameterization)

```{r}
include_graphics("Models/Appendix/FL/[DIR]Regr_Table.png")
```

### Table of Counsel Type Percentage Regression Results (Alternative Parameterization)

```{r}
include_graphics("Models/Appendix/FL/[ALT]Regr_Table.png")
```
...
```{r}
include_graphics("Models/Appendix/OR/[ALT]Regr_Table.png")
```
...
```{r}
include_graphics("Models/Appendix/PA/[ALT]Regr_Table.png")
```
...
## Florida Common & Alternative Parameterizations
...
## Oregon Common & Alternative Parameterizations
...
## Pennsylvania Common & Alternative Parameterizations
