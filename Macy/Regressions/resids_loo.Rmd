...
mod <- lm(
  car::logit(Public.Defender.conviction.rate) ~ 
    # Population
    Log.pop + Log.urban + Young.Males.Population + Log.Largest.Municipality.Population + High.School.Graduates +
    # Racial/Ethnicity
...
# We remove Log.Largest.Municipality.Population and Log.urban as they drop the Log.pop vif the most (to a reasonable value 6~)
mod2 <- lm(
  car::logit(Public.Defender.conviction.rate) ~ 
    # Population
    Log.pop + Young.Males.Population + #Log.urban + #Log.Largest.Municipality.Population + #High.School.Graduates +
    # Racial/Ethnicity
    African.American.Population + Native.American.Alaskan.Population + 
    Asian.or.Pacific.Islander.Population + Hispanic.or.Latino.Population +
    # Econ
...
cooks_plot <- function (mod, state, counsel) {
  ...
  ggplot(df, aes(x = obs, y = cooks)) +
    ...
    labs(
      title = paste0("Cook's Distance (", state, " - ", counsel, ")"),
      x = "Observation",
      y = "Cook's Distance"
    ) +
    theme_minimal() +
