...
```{r}
# choose 4 variables 
vars <- c("Below.Poverty.Line", "Number.of.Criminal.Court.Judges", "Unemployment.Rate", "Log.pop")

# baseline values for all predictors in the model
base_vals_PA$Unemployment.Rate <- mean(rdf_PA$Unemployment.Rate, na.rm = TRUE)
base_vals <- base_vals_PA

# helper to build predicted line for one variable
make_pred_df <- function(var_name) {
  x_seq <- seq(
    min(rdf_PA[[var_name]], na.rm = TRUE),
    max(rdf_PA[[var_name]], na.rm = TRUE),
    length.out = 200
  )
  ...
  pred <- predict(lm_p_PA, newdata = newdata, se.fit = TRUE)
...
# observed points
points_df <- rdf_PA %>%
  select(all_of(vars), Private.conviction.rate) %>%
  pivot_longer(
    cols = all_of(vars),
    names_to = "Variable",
    values_to = "x_value"
  ) %>%
  mutate(conviction_rate = Private.conviction.rate / 100)

points_df$Variable <- dplyr::recode(points_df$Variable,
         "Below.Poverty.Line" = "Poverty Rate", 
         "Number.of.Criminal.Court.Judges" = "Number of Criminal Court Judges",
...
ggplot() +
  geom_point(
    data = points_df,
    aes(x = x_value, y = conviction_rate),
    alpha = 0.6,
    size = 2
  ) +
  geom_ribbon(
  ...
  labs(
    title = "Pred CR vs selected predictors Pennsylvania (Private Counsel)",
    x = "Predictor Value",
    y = "Conviction Rate"
  ) +
  theme_minimal()
