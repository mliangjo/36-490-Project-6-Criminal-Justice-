  ...
  data_comb %>% filter(location_type == "county", measure_id == 120) %>%
    ...
    mutate(
      ...
      percent_sr = denominator[demographics == "defendants who self-represented"]/
        denominator[demographics== "all defendants"],
      percent_u = denominator[demographics == "defendants represented by an attorney of unknown type"]/
        denominator[demographics== "all defendants"],
      percent_o = denominator[demographics == "defendants represented by other attorney type"]/
        denominator[demographics== "all defendants"]
      ) %>% 
    distinct(location_id, .keep_all = TRUE) %>% 
    select(location_id, percent_p, percent_pd, percent_ca, percent_sr, percent_o, percent_u)
}
...
write_csv(full_pct_df_PA, "full_resp_PA.csv")
write_csv(full_pct_df_OR, "full_resp_OR.csv")
write_csv(full_pct_df_FL, "full_resp_FL.csv")
...
# PA - Private Regression Visual
```{r}
# choose 4 variables 
vars <- c("Below.Poverty.Line", "African.American.Population", "Unemployment.Rate", "Log.pop")

# baseline values for all predictors in the model
base_vals_OR$Unemployment.Rate <- mean(full_pct_df_OR$Unemployment.Rate, na.rm = TRUE)
base_vals <- base_vals_OR

# helper to build predicted line for one variable
make_pred_df <- function(var_name) {
  x_seq <- seq(
    min(full_pct_df_OR[[var_name]], na.rm = TRUE),
    max(full_pct_df_OR[[var_name]], na.rm = TRUE),
    length.out = 200
  )
  
  newdata <- base_vals[rep(1, length(x_seq)), ]
  newdata[[var_name]] <- x_seq
  
  pred <- predict(lm_p_OR, newdata = newdata, se.fit = TRUE)
...
pred_df <- bind_rows(lapply(vars, make_pred_df))

# observed points
points_df <- full_pct_df_OR %>%
  select(all_of(vars), percent_p) %>%
  pivot_longer(
    cols = all_of(vars),
    names_to = "Variable",
    values_to = "x_value"
...
points_df$Variable <- dplyr::recode(points_df$Variable,
         "Below.Poverty.Line" = "Poverty Rate", 
         "African.American.Population" = "Proportion African American",
         "Unemployment.Rate" = "Unemployment Rate",
         "Log.pop" = "Log Population"
         )

pred_df$Variable <- dplyr::recode(pred_df$Variable,
         "Below.Poverty.Line" = "Poverty Rate", 
         "African.American.Population" = "Proportion African American",
         "Unemployment.Rate" = "Unemployment Rate",
         "Log.pop" = "Log Population"
...
ggplot() +
  geom_point(
    data = points_df,
    aes(x = x_value, y = percent_p),
    alpha = 0.6,
    size = 2
  ) +
  geom_ribbon(
    data = pred_df,
    aes(x = x_value, ymin = lwr, ymax = upr),
    alpha = 0.2
  ) +
  geom_line(
    data = pred_df,
  ...
  labs(
    title = "Pred percent vs selected predictors Oregon (Private Counsel)",
    x = "Predictor Value",
    y = "Percent Private Counsel"
  ) +
  theme_minimal()
```
